...
public class PlayerController : MonoBehaviour
{
    ...
    public void CheckDashMidAir()
    {
        if (movementInputDirection != 0 && !isGrounded && isJumping && facingDirection == -1)
        {
            playerT.position = new Vector2((playerT.position.x - 3), playerT.position.y);
            shakeCamera.Shake();
        } else if (movementInputDirection != 0 && !isGrounded && isJumping && facingDirection == 1)
        {
            playerT.position = new Vector2((playerT.position.x + 3), playerT.position.y);
            shakeCamera.Shake();
        } else if ((movementInputDirection != 0 || movementInputDirection == 0) && isGrounded == true && facingDirection == -1)
        {
            playerT.position = new Vector2((playerT.position.x - 3), playerT.position.y);
            shakeCamera.Shake();
        } else if ((movementInputDirection != 0 || movementInputDirection == 0) && isGrounded == true && facingDirection == 1)
